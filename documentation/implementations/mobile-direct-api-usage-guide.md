...
## 🚨 **UPDATED: MOBILE-COMPATIBLE API ENDPOINTS NOW AVAILABLE**

**Update: The web app now provides mobile-compatible API endpoints for Stripe operations. You can now use:**

✅ **Available Endpoints:**
- `POST /api/stripe/create-payment-intent` - Create payment intents (mobile + web compatible)
- `POST /api/stripe/confirm-payment` - Confirm payments 
- `GET /api/stripe/payment-history` - Get payment history
- `POST /api/stripe/setup-intent` - Create setup intents
- `GET /api/courts` - Get courts data

⚠️ **Still Use Supabase Directly For:**
- User profile management (`/api/users/profile` remains web-only)
